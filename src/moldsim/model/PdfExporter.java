        ...
        try {
            ...
            if (!stats.isEmpty()) {
                ...
                document.add(new Paragraph(
                    "After " + stats.size() + ((stats.size() > 1) ? " weeks, " : " week, ") +
                    last.getInfectedCells() + ((last.getInfectedCells() == 1) ? " cell is infected (" : " cells are infected (") +
            ...
            // ── Footer ─────────────────────────────────────────
            document.add(Chunk.NEWLINE);
            Paragraph footer = new Paragraph("Generated by MoisSim — This report is an estimation tool, not a certified diagnosis.", SMALL_FONT);
